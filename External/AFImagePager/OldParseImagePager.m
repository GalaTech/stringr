...
@interface OldParseImagePager () <UIScrollViewDelegate>
{
    __weak id <OldParseImagePagerDataSource> _dataSource;
    __weak id <OldParseImagePagerDelegate> _delegate;
...
@implementation OldParseImagePager
...
- (void) loadData
{
    ...
    [stringQuery getObjectInBackgroundWithId:@"bW7gX8Zqni" block:^(PFObject *string, NSError *error) {
        ...
        [stringPhotoQuery findObjectsInBackgroundWithBlock:^(NSArray *photos, NSError *error) {
            ...
            if ([photos count] > 0) {
                [_scrollView setContentSize:CGSizeMake(_scrollView.frame.size.width * [photos count], _scrollView.frame.size.height)];
                ...
                for (int i = 0; i < [photos count]; i++) {
                    ...
                    PFImageView *imageView = [[PFImageView alloc] initWithFrame:imageFrame];
                    [imageView setBackgroundColor:[UIColor clearColor]];
                    [imageView setContentMode:[_dataSource contentModeForImage:i]];
                    [imageView setTag:i];
                    
                    PFFile *imageFile = [photos[i] objectForKey:kStringrPhotoPictureKey];
                ...
                [_countLabel setText:[NSString stringWithFormat:@"%d", [photos count]]];
                _pageControl.numberOfPages = [(NSArray *)photos count];
                _pageControl.hidden = ([(NSArray *)photos count] > 0 ? NO : YES);
            }
    ...
    NSArray *photoObjects = (NSArray *)[_dataSource arrayWithPhotoPFObjects];
    
    if ([photoObjects count] > 0) {
        [_scrollView setContentSize:CGSizeMake(_scrollView.frame.size.width * [photoObjects count], _scrollView.frame.size.height)];
        ...
        for (int i = 0; i < [photoObjects count]; i++) {
            ...
            PFFile *imageFile = [photoObjects[i] objectForKey:kStringrPhotoPictureKey];
            [imageView setFile:imageFile];
            [imageView loadInBackground];
