...
- (void)loadData
{
    ...
    if ([photoObjects count] > 0) {
        ...
        for (int i = 0; i < [photoObjects count]; i++) {
            ...
            UIActivityIndicatorView *loadingPhotoIndicator = [[UIActivityIndicatorView alloc] init];
            [loadingPhotoIndicator setActivityIndicatorViewStyle:UIActivityIndicatorViewStyleGray];
            [loadingPhotoIndicator setCenter:(CGPoint){imageView.frame.size.width / 2, imageView.frame.size.height / 2}];
            [loadingPhotoIndicator startAnimating];
            [imageView addSubview:loadingPhotoIndicator];
...
{    
    ...
    if([_pageControl currentPage] < ([[_dataSource arrayWithPhotoPFObjects] count] - 1)) {
